...
class BugBountyAPIConnectionError(RuntimeError):
    """Raised when the API client cannot connect to the Bug Bounty API server."""
...
class BugBountyAPIClient:
    ...
    def __init__(self, server_url: str, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        ...
        for i in range(MAX_RETRIES):
            try:
                logger.info(
                    f"🔗 Attempting to connect to Bug Bounty API at {server_url} "
                ...
                connected = True
                logger.info("🎯 Successfully connected to Bug Bounty API Server")
                logger.info(
                ...
                logger.warning(
                    f"🔌 Connection refused to {server_url}. "
                    "Make sure the Bug Bounty API server is running."
                )
                time.sleep(2)
        ...
        if not connected:
            error_msg = (
                f"Failed to establish connection to Bug Bounty API Server at "
                f"{server_url} after {MAX_RETRIES} attempts"
            )
            logger.error(error_msg)
            raise BugBountyAPIConnectionError(error_msg)
...
def setup_bug_bounty_mcp_server(api_client: BugBountyAPIClient) -> FastMCP:
    ...
    def amass_scan(
        ...
        if result.get("success"):
            ...
            logger.error("❌ Amass enumeration failed")
    ...
    def gobuster_scan(
        ...
        """Run Gobuster for directories, DNS, or vhosts with enhanced logging.
    ...
    def feroxbuster_scan(
        ...
        if result.get("success"):
            ...
            logger.error("❌ Feroxbuster scan failed")
    ...
    def katana_crawl(
        ...
        if result.get("success"):
            ...
            logger.error("❌ Katana crawling failed")
    ...
    def gau_discovery(
        ...
        """Run Gau to discover URLs from multiple sources with logging."""
        data = {
            "domain": domain,
            "providers": providers,
        ...
        if result.get("success"):
            ...
            logger.error("❌ Gau URL discovery failed")
    ...
    def waybackurls_discovery(
        ...
        if result.get("success"):
            ...
            logger.error("❌ Waybackurls discovery failed")
    ...
    def paramspider_mining(
        ...
        if result.get("success"):
            ...
            logger.error("❌ ParamSpider mining failed")
...
def main():
    ...
    logger.info("🚀 Starting Bug Bounty MCP Server")
    ...
    try:
        ...
        logger.info("🎯 Bug Bounty MCP Server ready for connections")
        mcp.run()

    except KeyboardInterrupt:
        logger.info("🛑 Bug Bounty MCP Server stopped by user")
        sys.exit(0)
